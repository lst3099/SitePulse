...
export default function DevicesPage({ role, lifecycleState, projectsRecords = mockData.projects, projectPeople = mockData.projectPeople, peopleRecords = mockData.people, registeredDevices: sharedRegisteredDevices, onRegisteredDevicesChange, authorizations, onDeviceChange, onOperationLog, registrationMode = false }) {
  ...
  const saveRegistration = (values) => {
    const next = { ...values, id: `device-local-${Date.now()}`, registered: true, projectId: undefined, online: true, permissionSync: 'notStarted', syncStatus: 'notStarted', personnelSync: 'notStarted', faceSync: 'notStarted', attendanceEnabled: false, entranceName: '未绑定', direction: 'both', platformPermission: 'deny', devicePermission: 'deny', difference: false, bindingStatus: '未绑定', lifecycleStatus: 'active', disabled: false, archived: false };
  ...
  const columns = [
    ...
    { title: '权限同步', key: 'sync', render: (_, device) => <div className="sync-stack"><SyncTag label="人员" value={device.personnelSync} /><SyncTag label="人脸" value={device.faceSync} /><SyncTag label="权限" value={device.permissionSync} /></div> },
    { title: '有效权限', key: 'effectivePermission', render: (_, device) => <StatusTag status={!device.projectId ? 'warning' : device.accessStatus === 'revoked' ? 'forbidden' : 'success'} label={!device.projectId ? '待绑定项目' : device.accessStatus === 'revoked' ? '已撤销' : '平台允许'} /> },
