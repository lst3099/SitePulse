...
import { buildProjectCreateLog, buildProjectEditLog, buildProjectLifecycleLog, normalizeUser, scopedProjects, statusLabel } from './pageUtils';
...
export function saveProjectChanges({ editing, values, onProjectLifecycle }) {
  const next = { ...(editing || {}), ...values, id: editing?.id || `project-local-${Date.now()}` };
...
export default function ProjectListPage({ role, lifecycleState, projectsRecords: sharedProjectsRecords, onProjectsRecordsChange, onProjectLifecycle, onOperationLog, onOpenProject }) {
  ...
  const openModal = (project) => {
    ...
    form.resetFields();
    form.setFieldsValue(project || { status: 'active', owner: '项目负责人甲' });
    setModalOpen(true);
  };
  ...
  return <div className="business-page"><PageHeader title="项目管理" description="管理项目范围、考勤规则与现场配置。停用或归档仅改变业务状态，历史原始事件与考勤结果保留并只读。" breadcrumb={['首页', '项目管理']} extra={canCreate && <Button type="primary" icon={<PlusOutlined />} onClick={() => openModal()}>新增项目</Button>} />
    ...
    <Modal title={editing ? '编辑项目' : '新增项目'} open={modalOpen} onCancel={closeModal} footer={null} destroyOnHidden><Form form={form} layout="vertical" onFinish={saveProject}><Form.Item label="项目名称" name="name" rules={[{ required: true, message: '请输入项目名称' }]}><Input /></Form.Item><Form.Item label="负责人" name="owner"><Input /></Form.Item><Form.Item label="状态" name="status"><Select options={[{ value: 'active', label: '进行中' }, { value: 'inactive', label: '已停用' }, { value: 'archived', label: '已归档' }]} /></Form.Item><Form.Item label="项目地址" name="address"><Input /></Form.Item><Button type="primary" htmlType="submit">保存</Button></Form></Modal>
  </div>;
}
